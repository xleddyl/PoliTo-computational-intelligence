...
# same as objects.py but better prints :)
...
class Quarto(object):
    ...
    def print(self, action: str, player: int = None, x: int = None, y: int = None, rounds: int = None):
        '''
        Print the board
        '''
        if not self.print_log:
            ...
            print(
                f"{type(self.__players[player]).__name__} selected piece: {self.__selected_piece_index}")
        elif action == 'placed':
            print(
                f"{type(self.__players[player]).__name__} placed piece {self.__selected_piece_index} at: {y, x}")
    ...
    def run(self) -> int:
        ...
        rounds = 0
        while winner < 0:
            self.print('round', rounds=rounds)
            rounds += 1
            piece_ok = False
            while not piece_ok:
                piece_ok = self.select(
                    self.__players[self._current_player].choose_piece())
            self.print('selected', self._current_player)
            piece_ok = False
            ...
            self.print('placed', self._current_player, x, y)
